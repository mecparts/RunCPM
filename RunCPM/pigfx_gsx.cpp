...
void GSX::escape() {
	switch (contrl[CONTROL_FUNCTIONID]) {
		...
		case ESC_GET_TEXT_CURSOR:	// inquire current current cursor address REQ for CRT
			contrl[CONTRL_INTOUT] = 2;
			intout[0] = 0; // ### getCursorY() / (TH_MIN + 1);
			intout[1] = 0; // ### getCursorX() / (TW_MIN + 1);
			break;
...
void GSX::read_colour_palette() {
	...
	intout[0] = clr_index;
	intout[1] = 0; // ### map((clr >> 11), 0, 0x1F, 0, 1000);
	intout[2] = 0; // ### map(((clr >> 5) & 0x3F), 0, 0x3F, 0, 1000);
	intout[3] = 0; // ###  map((clr & 0x1F), 0, 0x1F, 0, 1000);
}
